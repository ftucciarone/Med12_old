PROGRAM CORR_VECT
   ...
   DO WHILE ( jarg < iargc() )
      ...
      CASE('-v')
         IF ( jarg + 2 > iargc() ) THEN ! checking that there is at least 2 other arguments following
            PRINT *, 'ERROR: Missing input variables!' ; call usage_corr_vect()
         ELSE
            jarg = jarg + 1 ;  CALL getarg(jarg,cr)
            IF ( ANY(clist_opt == trim(cr)) ) THEN
               PRINT *, 'ERROR: Missing input varname 1 !' ; call usage_corr_vect()
            ELSE
               cv_out_U = trim(cr)
            END IF
            jarg = jarg + 1 ;  CALL getarg(jarg,cr)
            IF ( ANY(clist_opt == trim(cr)) ) THEN
               PRINT *, 'ERROR: Missing input varname 2 !' ; call usage_corr_vect()
            ELSE
               cv_out_V = trim(cr)
            END IF
         END IF
   ...
   IF ( .NOT. l_inv ) THEN
      ...
      IF ( i3d == 1 ) THEN
         ALLOCATE ( vdepth(nk) )
         CALL GETVAR_1D(cf_mm, 'gdept_1d', vdepth)
         IF ( cgrid_trg == 'U' ) THEN
      ...
      DO jt = 1, Ntr
         ...
         DO jk = 1, nk
            ...
            PRINT *, ''; PRINT *, 'Time step =', jt, ' Level =',jk ; PRINT *, ''
            ...
            rmiss_val = 9.96921e+36
            WHERE( .NOT. ABS(U_r8) .LT. 10. .OR. &
                .NOT. ABS(V_r8) .LT. 10. )
                Xdum8 = rmiss_val
                Ydum8 = rmiss_val
            ENDWHERE
         ...
         IF ( i3d == 1 ) THEN
            ...
            !! 3D:
            write(*,*) ' out 3d',trim(cf_out_U), trim(cv_out_U), ni_g, nj_g, nk, jt
            call wrnc3(cf_out_U, cv_out_U, ni_g, nj_g, nk, jt, U_c(:,:,:) )
            call wrnc3(cf_out_V, cv_out_V, ni_g, nj_g, nk, jt, V_c(:,:,:) )

         ELSE

            !! 2D
            write(*,*) ' out 2d',trim(cf_out_U), trim(cv_out_U), ni_g, nj_g, jt
            call wrnc2(cf_out_U, cv_out_U, ni_g, nj_g, jt, U_c(:,:,1))
            call wrnc2(cf_out_V, cv_out_V, ni_g, nj_g, jt, V_c(:,:,1))
...
SUBROUTINE usage_corr_vect()
   ...
   PRINT *,''
   PRINT *,' -i <x.nc> <y.nc>     =>  vector fields given in these 2 files'
   PRINT *,'                          to the same grid'
   PRINT *,''
   PRINT *,' -v <uvar> <vvar>     =>  vector fields in these 2 variables'
   PRINT *,''
   PRINT *,''
...
!!
SUBROUTINE wrnc3(cf,cv,ki,kj,kk,jt,U)
  USE NETCDF
  IMPLICIT NONE
  CHARACTER(LEN=*), INTENT(IN) :: cf,cv
  INTEGER :: ki,kj,kk
  INTEGER :: jt
  REAL(4), INTENT(IN), DIMENSION(ki,kj,kk) :: U
  INTEGER :: NCID,VID
  CALL CHECK( NF90_OPEN(CF,NF90_WRITE,NCID) )
  CALL CHECK( NF90_INQ_VARID(NCID,CV,VID) )
  CALL CHECK( NF90_PUT_VAR(NCID,VID,U,START=(/1,1,1,jt/),COUNT=(/ki,kj,kk,1/) ) )
  CALL CHECK( NF90_CLOSE(NCID) )
END SUBROUTINE wrnc3

SUBROUTINE wrnc2(cf,cv,ki,kj,jt,U)
  USE NETCDF
  IMPLICIT NONE
  CHARACTER(LEN=*), INTENT(IN) :: cf,cv
  INTEGER :: ki,kj
  INTEGER :: jt
  REAL(4), INTENT(IN), DIMENSION(ki,kj) :: U
  INTEGER :: NCID,VID
  CALL CHECK( NF90_OPEN(CF,NF90_WRITE,NCID) )
  CALL CHECK( NF90_INQ_VARID(NCID,CV,VID) )
  CALL CHECK( NF90_PUT_VAR(NCID,VID,U,START=(/1,1,jt/),COUNT=(/ki,kj,1/) ) )
  CALL CHECK( NF90_CLOSE(NCID) )
END SUBROUTINE wrnc2

...
!!!   USE stopack        ! Stochastic physics package
   ...
   LOGICAL         ::   ln_onlynight    ! flag to bound erp term 
...
CONTAINS
   ...
   SUBROUTINE sbc_ssr( kt )
      ...
      REAL(wp), ALLOCATABLE, DIMENSION(:,:) :: zz1, zz2
      !!----------------------------------------------------------------------
      !
...
!!!               IF( ln_stopack .AND. nn_spp_dqdt > 0 ) CALL spp_gen(kt, qrp,nn_spp_dqdt,rn_dqdt_sd,jk_spp_dqdt )
               ...
               IF(ln_onlynight) THEN
                       ALLOCATE(zz1(jpi,jpj), zz2(jpi,jpj))
                       zz1(:,:) = 1._wp
                       IF(istart.eq.1) THEN
                               nday_qsr = -1
                               istart=0
                       ENDIF
                       zz2(:,:) = sbc_dcy( zz1(:,:), l_mask = .TRUE. )
                       qrp(:,:) = qrp(:,:) * zz2(:,:)
                       DEALLOCATE( zz1, zz2 ) 
               ENDIF
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'namsbc_ssr in configuration namelist' )
      ...
      IF(lwp) THEN                 !* control print
         ...
         WRITE(numout,*) '      Only night correction for SST relaxation ln_onlynight = ', ln_onlynight
      ENDIF
      !

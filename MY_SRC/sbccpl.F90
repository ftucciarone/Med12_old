MODULE sbccpl
   ...
   USE diurnal_bulk , ONLY : ln_diurnal, x_dsst
   USE cool_skin    , ONLY : x_csdsst
...
!!$
      ...
      IF( srcv(jpr_mslp)%laction ) THEN                    ! UKMO SHELF effect of atmospheric pressure on SSH 
          ...
          IF( kt == nit000 .AND. .NOT. ln_apr_dyn) ssh_ibb(:,:) = ssh_ib(:,:)  ! correct this later (read from restart if possible) 
      END IF 
      !
...
!!         END SELECT
   ...
   SUBROUTINE sbc_cpl_snd( kt )
      ...
      IF( ssnd(jps_toce)%laction .OR. ssnd(jps_tice)%laction .OR. ssnd(jps_tmix)%laction ) THEN
         ...
         IF( nn_skin_sst .NE. 0 ) THEN
           SELECT CASE ( nn_skin_sst )
             CASE ( 1 ) ; ztmp1 = ztmp1 + x_csdsst + x_dsst  ! Both skin and cool layers
             CASE ( 2 ) ; ztmp1 = ztmp1            + x_dsst  ! Only skin layer
             CASE ( 3 ) ; ztmp1 = ztmp1 + x_csdsst           ! Only cool layer
             CASE default ; CALL ctl_stop( 'sbc_cpl_snd: wrong nn_skin_sst' )
           END SELECT
         ENDIF
